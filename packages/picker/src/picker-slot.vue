...
import { once, addClass, removeClass } from 'mintUi/src/utils/dom'
import emitter from 'mintUi/src/mixins/emitter'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    translate2Value (translate) {
      // 移动后，对应列表的第几个
      var itemHeight = this.itemHeight
      // translate:移动-36 / 组件的高度 * 组件高度
      translate = Math.round(translate / itemHeight) * itemHeight
      // translate:移动后的值 （36 - 组件默认个数/ 2 * 组件高度） / 组件高度
